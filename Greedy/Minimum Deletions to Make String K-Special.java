Q:- https://leetcode.com/problems/minimum-deletions-to-make-string-k-special/
*******************************************************************************************************
...
-----------------------------------------------------------
  class Solution {
    public int minimumDeletions(String word, int k) {
        ...
        {
            int minFreq = freq[i];
            int deleted = deletedTillNow; //temp taken to find deletion for j = 25 to j > i
            for(int j = 25; j > i; j--)
            {
                if(freq[j] - freq[i] <= k)
                    break;

                 deleted += freq[j] - minFreq - k;   
            }
